...
func (f *Fuzz) Before(params string, p *services.ProviderService, r *http.Request, reqJson styles.PartialJSON) (styles.PartialJSON, error) {
	...
	if p == nil {
		plugins.Logger.Debug("fuzz plugin skipped - no provider context")
		return reqJson, nil
	}
	...
	if !ok {
		plugins.Logger.Debug("fuzz list_models command type assertion failed")
		return reqJson, nil
	}

	plugins.Logger.Debug("fuzz fetching models from provider")
	models, err := cmd.DoListModels(p, r)
	if err != nil {
		plugins.Logger.Debug("fuzz failed to list models", zap.Error(err))
		return reqJson, nil
	}
